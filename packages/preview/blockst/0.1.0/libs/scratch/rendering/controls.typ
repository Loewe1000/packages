...
#import "colors.typ": scratch-block-options, get-colors-from-options, get-stroke-from-options, get-font-from-options, colors-normal
...
#let conditional-block(
  ...
  // Pre-resolved theme params — passed from render-block to avoid extra context per nesting level
  colors: none,
  stroke-thickness: none,
  font-family: none,
) = context {
  // Only read state for params not already provided by the caller
  let options = if colors == none or stroke-thickness == none or font-family == none {
    scratch-block-options.get()
  } else { none }
  let resolved-colors = if colors != none { colors } else { get-colors-from-options(options) }
  let resolved-stroke = if stroke-thickness != none { stroke-thickness } else { get-stroke-from-options(options) }
  let resolved-font = if font-family != none { font-family } else { get-font-from-options(options) }
  ...
  // All measure() calls in the same outer context — no inner #context [...] needed.
  // This removes one ContextElem per nesting level, preventing "maximum show rule depth exceeded".
  ...
  block(
    ...
    #set text(font: resolved-font, resolved-colors.text-color, weight: 500)

    // Draw header and body
    #place(top + left, dy: block-offset-y)[
      #curve(
        fill: resolved-colors.control.primary,
        stroke: (paint: resolved-colors.control.tertiary, thickness: resolved-stroke),
...
// Repeat n times
// ------------------------------------------------
#let repeat(count: 10, body: none, labels: (repeat: "repeat", times: "times"), colors: none, stroke-thickness: none, font-family: none) = {
  let resolved-colors = if colors != none { colors } else { colors-normal }
  conditional-block(
    [#stack(dir: ltr, spacing: 1.5mm, labels.repeat, number-or-content(count, resolved-colors.control), labels.times)],
    first-body: body,
    colors: colors,
...
// ------------------------------------------------
#let repeat-until(condition, body: none, labels: (repeat-until: "repeat until"), colors: none, stroke-thickness: none, font-family: none) = {
  let resolved-colors = if colors != none { colors } else { colors-normal }
  conditional-block(
    [#stack(dir: ltr, spacing: 1.5mm, labels.at("repeat-until"), if condition != [] { condition } else { _condition-fn(colorschema: resolved-colors.control, []) })],
    first-body: body,
    colors: colors,
    stroke-thickness: stroke-thickness,
    font-family: font-family,
...
// ------------------------------------------------
#let repeat-forever(body, labels: (forever: "forever"), colors: none, stroke-thickness: none, font-family: none) = conditional-block(
...
#let if-then-else(
  ...
  // No own context here — colors/stroke/font are provided by the caller (render-block).
  // When called standalone without colors, conditional-block reads state via its own context.
  let resolved-colors = if colors != none { colors } else { colors-normal }
  conditional-block(
    [#stack(dir: ltr, spacing: 1.5mm, labels.at("if-then"), if condition != [] { condition } else { _condition-fn(colorschema: resolved-colors.control, []) }, labels.then)],
    first-body: then,
